...
% This section covers if the previously derived equations can be solved for given initial parameters. It is worth noting that the TOV equation presented in \eqref{3-Mass-TOV-Eq} is initially defined only for $r>0$ but can be extended to $r=0$. The limiting values can be calculated using L'Hôpital's rule together with the initial values $m_0=0$, $0\leq p_0,\rho_0<\infty$ and read $m'(0)=0$ and $p'(0)=0$.
