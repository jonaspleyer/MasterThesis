\begin{section}{Calculating the Mass of a Star with an \texorpdfstring{\acrNoHyperlink{\acs}{eos}}{EoS}}
\begin{subsection}{Deriving the \texorpdfstring{\acrNoHyperlink{\acs}{TOV}}{TOV}-Equation}
\label{3-Mass-Sec-TOVDerivation}
This chapter derives the \ac{TOV} equation which was introduced earlier in %TODO reference section 01-Introduction with GR references.
...
\begin{align}
	-8\pi T_0^0 = 8\pi\rho &= \frac{\lambda'\e^{-\lambda}}{r}+\frac{1-\e^{-\lambda}}{r^2}\label{3-Mass-Equ-EinstEqu-1}\\
	8\pi T_1^1 = 8\pi p &= \nu'\frac{\e^{-\lambda}}{r} - \frac{1-\e^{-\lambda}}{r^2}\label{3-Mass-Equ-EinstEqu-2}\\
	8\pi T_2^2 = 8\pi p &= \frac{\e^{-\lambda}}{2}\left[\nu''+\left(\frac{\nu'}{2} + \frac{1}{r}\right)\left(\nu'-\lambda'\right) \right]\label{3-Mass-Equ-EinstEqu-3}
\end{align}
Since $R_4^4=R_3^3$, we omitted the last equation.
From equation \eqref{3-Mass-Equ-EinstEqu-1} we infer the relation.
\begin{equation}
	\e^{-\lambda} = 1 - \frac{2}{r}\int\limits_0^r 4\pi\rho(r') r'^2dr' = 1 - \frac{2m(r)}{r}.
	\label{3-Mass-Equ-MassDepOnr}
\end{equation}
The metric needs to be defined at every point in space and thus can not have any additional integration constant in equation \eqref{3-Mass-Equ-MassDepOnr}, since otherwise we would obtain a term $a/r$ which is not defined for $r\rightarrow0$.\\
...
In addition to the Field equations \eqref{3-Mass-Equ-EinstEqu-1} through \eqref{3-Mass-Equ-EinstEqu-3} the divergence of the Stress-Energy Tensor yields more information
\begin{equation}
	\nabla_\mu T^{\mu\nu}=0.
\end{equation}
...
\end{align}
Together with equation \eqref{3-Mass-Equ-EinstEqu-2} and the definition \eqref{3-Mass-Equ-MassDepOnr}, we can write
\begin{align}
	...
	\label{3-Mass-Equ-TOV-Eq-1}
\end{align}
where in the last step the constants $c=G=1$ were put back in. Equation \eqref{3-Mass-Equ-TOV-Eq-1} together with \eqref{3-Mass-Equ-MassDepOnr} yields the \ac{TOV} differential equations
\begin{alignat}{3}
...
\begin{equation}
	...
	\label{3-Mass-Equ-TOVNonRel-Limit}
\end{equation}
...
\end{equation}
by using the definition of our polytropic \ac{eos} and equation \eqref{3-Mass-Equ-TOVNonRel-Limit}. Rearranging and taking the derivative of this equation and using $\partial m/\partial r = 4\pi\rho r^2$, we obtain
\begin{equation}
...
\begin{equation}
	\frac{1}{\xi^2}\frac{\partial}{\partial\xi}\left(\xi^2\frac{\partial\theta}{\partial\xi}\right) + \theta^n=0 \label{3-Mass-Equ-Lane-Emden-Eq}
\end{equation}
Exact solutions are known for the cases $n=0,1,5$. The derivation can be found in appendix \ref{99-App-A-Exact-LE-Solutions}. Figure \ref{3-Mass-Plt-LE-Exact-Results-Plots} and Table \ref{3-Mass-Tbl-LE-Exact-Results} summarise them. Together with equation \eqref{3-Mass-Equ-Lane-Emden-Eq}, we can already suspect that for values $n\geq5$, the equation does not yield solutions with a zero value.\\
% \begin{figure}[H]
% 	\centering
...
\begin{minipage}[t]{0.45\textwidth}
	\begin{figure}[H]
		\captionof{table}[Lane Emden exact Solutions]{\ac{LE} exact Solutions and their zero value.}
		\label{3-Mass-Tbl-LE-Exact-Results}
	\end{figure}
\end{minipage}
%
%
...
\end{lemma}
\begin{proof}
	Taking the derivative of $\rho_{av}$ and applying equation \eqref{3-Mass-Equ-MassDepOnr}, we obtain 
	\begin{equation}
...
\end{theorem}
\begin{proof}
	In our attempt to obtain an upper limit for the mass of a spherically symmetric star, we start by taking the difference of equation \eqref{3-Mass-Equ-EinstEqu-2} and \eqref{3-Mass-Equ-EinstEqu-3}, we obtain 
	\begin{align}
	...
	\begin{equation}
		...
		\label{3-Mass-Equ-MassLimitApprox1}
	\end{equation}
	and with the explicit solution for $\e^{-\lambda}$, we obtain
	\begin{equation}
		\left.\frac{2m(r)}{r}\right|_R = \frac{2M}{R}.
	\end{equation}
	When plugging this into equation \eqref{3-Mass-Equ-MassLimitApprox1}, the result is
	\begin{equation}
	...
	\begin{equation}
		...
		\label{3-Mass-Equ-MassLimitNuApprox}
	\end{equation}
