...
\end{align}
Since $R_4^4=R_3^3$, we omitted the last equation. From equation \eqref{3-Mass-Equ-EinstEqu-1} we infer the relation.
\begin{equation}
	\e^{-\lambda} = 1 - \frac{2}{r}\int\limits_0^r 4\pi\rho(r') r'^2dr' = 1 - \frac{2m(r)}{r}.
	\label{3-Mass-Equ-MassDepOnr}
\end{equation}
The metric needs to be defined at every point in space and thus we can not have any additional integration constant in equation \eqref{3-Mass-Equ-MassDepOnr}, since otherwise we would obtain a term $a/r$ which is ill defined for $r\rightarrow0$.\\
...
In addition to the Field equations \eqref{3-Mass-Equ-EinstEqu-1} to \eqref{3-Mass-Equ-EinstEqu-3} the divergence of the Stress-Energy Tensor yields more information
\begin{equation}
	\nabla_\mu T^{\mu\nu}=0.
\end{equation}
