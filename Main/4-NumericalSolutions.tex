...
In order to solve the equations, a transformation in the form applied in section \ref{3-Mass-LEDerivation} is used since it was hoped to increase the precision with which the zero value is determined.
This can be motivated by looking at plots for \ac{LE} solutions as in figure \ref{3-Mass-LE-Exact-Results-Plots} and comparing them to \ac{TOV} results of figure \ref{4-NumSol-Plt-TOVEqEasyEOS}.
...
When transforming back by using $\kappa\xi=r$ as explained in section \ref{3-Mass-LEDerivation} one should note that $\kappa$ depends on $n$ via 
\begin{equation}
