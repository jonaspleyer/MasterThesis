...
\end{align}
as derived previously in section~\ref{sec:3-Mass-Sec-TOVDerivation} will be discussed.
...
The Lane-Emden equation was obtained in section~\ref{sec:3-Mass-Sec-LEDerivation} as the non-relativistic limit of the \ac{TOV} equation by neglecting terms of order $1/c^2$ and higher and setting $G=c=1$.
...
With the conversion factor $\kappa$ derived in section~\ref{sec:3-Mass-Sec-LEDerivation}, \ac{TOV} and \ac{LE} results can be compared.
...
In order to solve the equations, a transformation in the form applied in section~\ref{sec:3-Mass-Sec-LEDerivation} is used since it was hoped to increase the precision with which the zero value is determined.
...
When transforming back by using $\kappa\xi=r$ as explained in section~\ref{sec:3-Mass-Sec-LEDerivation} one should note that $\kappa$ depends on $n$ via
\begin{equation}
