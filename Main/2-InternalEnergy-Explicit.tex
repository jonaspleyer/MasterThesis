\begin{section}{Thermodynamical Calculation of an EoS}
This chapter aims at developing an fully special relativistic equation of state of a noninteracting gas. We briefly summarise important concepts necessary for the derivation.\\
...
The last equation shows that to fully calculate $U$ it is necessary to obtain a readable output from the partition function $\Z$.
%
%
%
...
\begin{align}
	\Z 	&= \frac{V^N}{N!h^{3N}}\left[\int\limits_{\R^3}\exp\left(- \beta H(p)\right)d^3p\right]^N\\
...
\begin{align}
	\Z 	&= \frac{V^N}{N!h^{3N}}\left[\int\limits_{\R^3}\exp\left(- \frac{mc^2\sqrt{1+\frac{p^2}{m^2c^2}}}{k_BT}\right)d^3p\right]^N\label{2-IntEner-EQ1}\\
...
This section aims to develop an equation between the thermodynamic energy density $\rho=\U/V$ and the pressure $p$ of the gas given by the ideal gas equation \eqref{2-IntEner-IdealGasEq}. 
We assume an additional constraint and thus further reduce the degrees of freedom of the thermodynamic system.\\
When assuming an adiabatic condition $\delta Q=0$ and using the First Law of Thermodynamics \cite{fliessbachStatistischePhysikLehrbuch2018} $dU = \delta Q + \delta W$, where $\delta W=-pdV$ and $dU=C_VdT$, we can relate pressure and temperature. 
This adiabatic condition is quite well satisfied since radiational losses compared to the total amount of mass can be neglected.
Using equation \ref{2-IntEner-InternelEnergyExplicit} and \ref{2-IntEner-IdealGasEq}, we obtain
\begin{alignat}{3}
    -pdV &= C_VdT\\
