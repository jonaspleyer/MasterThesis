...
This chapter aims at developing an fully special relativistic equation of state.

In the canonical ensemble the internal energy is obtained by the equation $\F=\U+TS$ where all quantities present can be obtained by means of the partition function.
Microscopically the partition function is given by the behaviour of the $N$ particles determined by the Hamiltonian $H$. For concepts and definitions of Hamilton Mechanics the reader is referred to \cite{eschrigTopologyGeometryPhysics2011, fliessbachMechanikLehrbuchZur2020, spivakPhysicsMathematiciansMechanics2010} and details about thermodynamics can be found in \cite{fliessbachStatistischePhysikLehrbuch2018}. In general we assume $H:\R^N\times\R^N\rightarrow\R$ to be a positive smooth function that assigns the positions and momenta of $N$ particles an energy.
...
\begin{align}
	\Z(T,V,N) 	&= \int\limits_{\R^{2N}}\exp\left(-\frac{H(x_1,\dots,x_N,p_1,\dots,p_N)}{k_BT}\right)\frac{dx_1dp_1\dots dx_Ndp_N}{N!h^{3N}}\\
